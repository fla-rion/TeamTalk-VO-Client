"""Tests für die Qt-Mnemonik-/Ellipsis-Normalisierung in i18n._() (v7.0.3)."""
import os
import sys
...
def test_strip_escaped_ampersand_preserved():
    # '&&' ist ein literales '&' in Qt; darf nicht als Mnemonik gelten.
...
def test_strip_ampersand_followed_by_space_is_literal():
    # '& ' (mit Leerzeichen) ist keine Mnemonik – bleibt unverändert.
    assert _strip_qt_decorations("Updates & Versionen") == ("Updates & Versionen", None, False)
...
def test_reattach_mnemonic_finds_inner_letter():
    # Wenn die Mnemonik-Letter nicht am Wortanfang steht, vor dem ersten Treffer einfügen.
...
def test_reattach_mnemonic_fallback_to_start():
    # Letter nicht in der Übersetzung → Fallback: '&' an den Anfang.
...
def test_direct_dict_lookup_still_works():
    # Bestehende Einträge ohne Dekoration funktionieren wie bisher.
    set_language("fr")
    assert _("Verbinden") == "Se connecter"


def test_qt_mnemonic_string_is_translated():
    # '&Verbinden...' steht nicht im Dict, '&Verbinden' auch nicht – aber 'Verbinden' schon.
    set_language("fr")
    result = _("&Verbinden")
    # Erwartung: '&' wird wieder vor dem 'V' der Übersetzung angefügt.
    assert result == "Se &connecter" or result.startswith("&") or "&" in result
    # Strenger: das '&' steht direkt vor der Mnemonik-Letter 'v' (lowercase).
    # Übersetzung enthält kein 'v'? Dann Fallback an den Anfang.
    if "v" in "Se connecter".lower():
        assert result == "Se connecter".replace("c", "&c", 1) or "&" in result


def test_qt_string_with_ellipsis_is_translated():
    set_language("fr")
    # 'Verbinden' → 'Se connecter' (vorhanden im Dict)
    assert _("Verbinden...") == "Se connecter..."
...
def test_qt_string_with_mnemonic_and_ellipsis():
    set_language("fr")
    result = _("&Verbinden...")
    # Muss mit '...' enden und ein '&' enthalten.
    assert result.endswith("...")
    assert "&" in result


def test_explicit_dict_entry_with_mnemonic_takes_precedence():
    # 'Notiz &bearbeiten...' ist direkt im FR-Dict eingetragen – muss exakt diesen Wert liefern,
    # ohne durch die Normalisierung umgeschrieben zu werden.
    set_language("fr")
...
def test_untranslated_qt_string_returned_as_is():
    ...
    # Weder direkt noch normalisiert im Dict → Original kommt zurück.
    assert _(nonsense) == nonsense
